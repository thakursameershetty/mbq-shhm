...
const { getKeyPoolStatus, attemptSmartBulkMatchWithAI, generateChatResponse } = require('./aiMapping');
...
app.post('/api/auth/register', async (req, res) => {
  ...
  try {
    ...
    // Create the user. Phenotypic data is collected later via the in-app
    // questionnaire (see PatientSurveyModal / /api/users/:id/report-answers),
    ...
    console.log(`🧬 User #${newUserId} registered.`);
...
// ─────────────────────────────────────────────────────────────────────────────
// Lifestyle Questionnaire (Activity / Sleep) Route
// ─────────────────────────────────────────────────────────────────────────────
app.post('/api/users/:id/lifestyle', async (req, res) => {
  const userId = req.params.id;
  const { dailyActivity, sleepTiming } = req.body;

  if (!dailyActivity || !sleepTiming) {
    return res.status(400).json({ error: 'dailyActivity and sleepTiming are required.' });
  }

  try {
    const query = `
      UPDATE users
      SET phenotypic_analysis = COALESCE(phenotypic_analysis, '{}'::jsonb) || jsonb_build_object(
        'personal_profile',
        COALESCE(phenotypic_analysis->'personal_profile', '{}'::jsonb) || jsonb_build_object(
          'dailyActivity', $1::text,
          'sleepTiming', $2::text
        )
      )
      WHERE id = $3
    ...
    console.error('Error saving lifestyle answers:', err);
    res.status(500).json({ error: 'Failed to save lifestyle answers.' });
  }
});
...
app.post('/api/users/:id/report-answers', express.json(), async (req, res) => {
  ...
  try {
    ...
    // long the LLM call takes. Vercel keeps this function alive exactly as
    // long as the response is pending, so splitting into two sequential
    // awaited requests from the client is safe here in a way a true
    // fire-and-forget background task after res.json() would not be.
...
app.post('/api/users/:id/generate-report', express.json(), async (req, res) => {
  ...
  try {
    ...
    const { notFound, value } = await withUserRowLock(userId, async (client, user) => {
      ...
      // Without this, generateAiReportForPanel's early-return guard (ai_report already
      // set) silently keeps whatever report was generated under the panel's *previous*
      // gene selection (e.g. a Pro EDAR+FGFR2 report left in place after the panel was
      // downgraded to a Lite FGFR2-only test), instead of regenerating it against the
      // current variants.
      if (panelData) {
